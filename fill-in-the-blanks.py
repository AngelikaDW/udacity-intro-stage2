...
easy_quiz = """Fill in the missing terms:\n 
    ...
    s. ___1___ ( ), s.upper() -- returns the lowercase or uppercase version 
    of the string.\n 
    s. ___2___ ()) -- returns a string with whitespace removed from the 
    start and end \n
    s. ___3___ ('other'), s.endswith('other') -- tests if the string starts 
    or ends with the given other string \n 
    s. ___4___ ('other') -- searches for the given other string (not a 
        regular expression) within s, and returns the first index where 
    it begins or -one if not found \n 
    ...
    convenient special case s.split() (with no arguments) splits on all 
    whitespace chars.\n"""
...
medium_quiz = '''Medium level: Fill in the missing terms:\n
    ...
    and like lists they are one of the most commonly used data structures in 
    programming. A dictionary is used to ___2___ or associate things you 
    want to store to   ___3___   you need to get them. The contents of a dict
    can be written as a series of key:value pairs within ___4___ braces
    { }.\n'''

medium_answers = ['dict', 'map', 'keys', 'curly']

hard_quiz = '''Hard level. Fill in the missing terms:\n
    The power of regular  ___1___  is that they can specify patterns, not just
     fixed  ___2___ . Here are the most basic patterns which match single 
     chars:\n .(a period) -- matches any single character except newline '\n
     ' \w -- (lowercase w) matches a "  ___3___  " character: a letter or 
     digit or underbar [a-zA-Z0-9_]. Note that although "word" is the mnemonic
     for this, it only matches a single word char, not a whole word. \W 
    (upper case W) matches any non-word character.  ___4___  = start, 
...
def prep_palceholders(level, answers):
    """
    Behavior: creates list of placeholders 
    Input: difficulty level and list of answers, 
    OUTPUT: list of placeholders as found in quiz text
    """
...
def replace_userguess(quiz, word, user_guess):
    """
    Input quiz, word that is currently iterated, user_guess.
    Behavior: replaces placeholder/word with user guess.
    Output: quiz as a list.
    """
    loc = quiz.index(word)
    quiz.remove(word)
...
def validate_userguess(user_guess, answers, index, nbr_games):
    """
    Behavior: validates user input by comparing to answers, for a set number 
    of times.
    OUTPUT: returns user_guess and the index to jump to next answer.
    """
    if user_guess == answers[index]:
        ...
        if nbr_games == 1:
            ...
            The correct_answer was:___", answers[index], "__!\n"
            exit()    
        else:
...
def write_game(quiz, placeholders, user_nbr_games, answers):
    """
    Behavior: iterates through the quiz[], compares if current word is in the 
    placeholders[] asks for user input for question.
    OUTPUT: returns the quiz as a []     
    """     
    index = 0
    for word in quiz:
        if word in placeholders:
            ...
            while nbr_games > 0:
                ...
                if answer != None: #correct answer next placeholder 
                    print replace_userguess(quiz, word, answer)
                    index = index +1
                    break
                elif answer == None:
...
def play_game():
    """
    Behavior: runs the game. Asks for user input on
    difficulty level, number of guesses.
    OUTPUT: if guesses are correct: quiz with answers filled in.
    """     
    print "\nHello! Let's play a Python quiz."
    difficulty_level= raw_input("Which level do you want to play?\
    ...
    print quiz
    print placeholders
    print "Cheat-mode on: Anwsers are:", answers
    ##Comment on == Cheat mode off, Comment off == Cheat mode on
    quiz = quiz.split() # quizin a []
